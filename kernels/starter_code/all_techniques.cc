    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                auto raw_w_low = _mm256_and_si256(raw_w, lowMask);
                auto raw_w_high = _mm256_and_si256(lowMask, _mm256_srli_epi16(raw_w, 4));
                auto raw_w_next_low = _mm256_and_si256(raw_w_next, lowMask);
                auto raw_w_next_high = _mm256_and_si256(lowMask, _mm256_srli_epi16(raw_w_next, 4));
                ...
                w_0_next = _mm256_sub_epi8(raw_w_next_low, zero_point);
                w_128_next = _mm256_sub_epi8(raw_w_next_high, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    auto m = A->row, n = B->column, k = A->column;
    ...
    for (int i = 0; i < num_thread; i++) {
        ...
        // end = split * (i + 1);
        threads_args[i].start_j = start;
        threads_args[i].end_j = end;
